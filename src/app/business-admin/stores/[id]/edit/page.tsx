  ...
  useEffect(() => {
    const fetchStore = async () => {
      ...
      try {
        ...
        if (data.manager) {
          try {
            const res = await fetch(`/api/users/${data.manager}`);
            if (res.ok) {
              const user = await res.json();
              setManagerName(user.first_name ? `${user.first_name} ${user.last_name || ''}` : user.username || data.manager);
            }
          } catch {}
        }
      } catch (err) {
      ...
      {loading ? (
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            {managerName && <p className="text-gray-500 text-sm">Current: {managerName}</p>}
          </div>
